...
if($AllMarketProducts){
	foreach($AllMarketProducts as $m_d){
		...
		if($productDiscount){$moneyFormatDiscount = $Dot->Dot_restyle_text($productDiscount); $hasDiscount = '<div class="produc_discount">'.$moneyFormatDiscount.' '.$product_Currency.'</div>';} 
		echo '
		<!--Product STARTED-->
